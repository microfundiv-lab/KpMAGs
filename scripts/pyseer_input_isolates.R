...
# metadata filter
metadata = metadata[which(!is.na(metadata$Country) & !is.na(metadata$Health_Status) & metadata$Genome_Type == "Isolate"),]
...
# save new table
write.table(pa.fi, file="gwas/diseased-all_isolates/pyseer_input.tsv", quote=FALSE, row.names=FALSE, sep="\t")
